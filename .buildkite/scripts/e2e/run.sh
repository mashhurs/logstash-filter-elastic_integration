...
./e2e/resolve_stack_version.sh

# download latest released elastic-package
./e2e/download_elastic_package.sh $1 # the platform can be either macos or debian
...
################### Elastic stack #############################
# run ES, KB and ERP as a daemon
./elastic-package stack up -d --version "${ELASTIC_STACK_VERSION}" --services elasticsearch,package-registry,kibana,logstash,fleet-server,elastic-agent
#./elastic-package stack up --version 8.11.0 --services elasticsearch,package-registry,kibana,logstash,fleet-server,elastic-agent &

sleep 10 # give sometime to spin up
...
docker cp e2e/config/logstash.conf logstash-container:/usr/share/logstash/pipeline
docker cp e2e/config/logstash.yml logstash-container:/usr/share/logstash/config
...
docker cp e2e/config/elastic-agent.yml elastic-agent-container:/usr/share/elastic-agent
